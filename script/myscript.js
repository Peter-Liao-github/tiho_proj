$(window).scroll(function () {
  ...
  function appear(block, fadeInStartPoint, fadeOutStartPoint) {
    ...
    if (
      scrolled > fadeInStartPoint &&
      scrolled <= fadeInStartPoint + fadeInEndPoint
    ) {
    ...
    if (
      scrolled > fadeInStartPoint + fadeInEndPoint &&
      scrolled <= fadeInStartPoint + fadeOutStartPoint
    ) {
      slopeOp = 0
      slopeBtm = 0
      $('.' + block + ' .appear').css({
    ...
    if (
      scrolled > fadeInStartPoint + fadeOutStartPoint &&
      scrolled <= fadeInStartPoint + fadeOutEndPoint
    ) {
  ...
  if (scrolled > 5.4) {
    $('.rules-block-bgi>img:first-child').css(
      'top',
      -10 - (scrolled - 5.4) * 50 + 'vh'
    )
    $('.rules-block-bgi>img:nth-child(3)').css(
      'top',
      -10 - (scrolled - 5.4) * 50 + 'vh'
    )
  }
...
$(function () {
  ...
  //關於區圖文切換
  var aboutImgOrigin = [
    'img/TIHO_Web_Ingredient-22.png',
    'img/TIHO_Web_Ingredient-23.png',
    'img/TIHO_Web_Ingredient-24.png',
    'img/TIHO_Web_Ingredient-25.png'
  ]
  var aboutImg = [
    'img/TIHO_Web_Ingredient-26.png',
    'img/TIHO_Web_Ingredient-27.png',
    'img/TIHO_Web_Ingredient-28.png',
    'img/TIHO_Web_Ingredient-29.png'
  ]
  var aboutTextTitle = [
    'Price advantage',
    'Quality advantage',
    'International service',
    'Material- lifelong warranty'
  ]
  var aboutTextP = [
    'The introduction of cryptocurrency allows everyone to be both consumer and investor to enjoy low-cost price for dental implant.',
    'The quality control by one-stop service from the interference ofupstream Japanese brands to medical technology at the clinic anddental medical service as well as to the manufacturing industry of denturefactory . It is excellent in the industry.',
    'Dental implant in one clinic allows you to enjoy international service with no concern of being left in the dark.',
    'Material- lifelong warranty surgery fee and denture restoration feewill be charged separately.'
  ]
  ...
  function blockInnerSwitch(t, i, j) {
    ...
    function innerSwitch(t, i, j) {
      ...
      $('.' + t + ' .option-' + j).css('opacity', 0.4)
      $('.' + t + ' .opt-dot').css({
        borderColor: '#555',
        backgroundColor: '#bbb'
      })
      $('.' + t + '-content .option-' + j + '.show-content').css(
        'display',
        'none'
      )
      ...
      $('.' + t + '-content .option-' + i + '.show-content').css(
        'display',
        'block'
      )
